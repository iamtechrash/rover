...
// Python server (change to your PC IP)
...
void led1on(){
  ...
  Serial.println("Camera1 command received");
...
void led2on(){
  ...
  Serial.println("Camera2 command received");
...
void loop(){
  ...
  if(compareUID(rfid.uid.uidByte,post1UID)){
    ...
    Serial.println("Post1 Tag Detected");
  ...
  if(compareUID(rfid.uid.uidByte,post2UID)){
    ...
    Serial.println("Post2 Tag Detected");
